...
class LocationService {
  ...
  Future<void> prepareLocationListener() async {
    print("LocationService: prepareLocationListener called.");
    try {
      await handlePermissionRequest();
      print("LocationService: Permission request handled successfully. Starting location listener...");
      _startLocationListner();
    } catch (error) {
      print("LocationService: Error in prepareLocationListener/handlePermissionRequest: $error");
    }
  }
...
//
  ...
  syncLocationWithFirebase(Location currentLocation) async {
    ...
    if (AppService().driverIsOnline) {
      ...
      int isFree = 1;
      if (driverLocationDocs.exists && driverLocationDocs.data() != null) {
        final data = driverLocationDocs.data()!;
        if (data.containsKey("free")) {
          isFree = int.tryParse(data["free"].toString()) ?? 1;
        }
      }
